 ...
 * @LastEditTime: 2022-10-26 15:38:11
...
<template>
  <PageWrapper contentFullHeight>
    <div v-if="viewInfo">
      ...
      <a-row :gutter="[10, 0]">
        <a-col v-for="(item, index) in schemasData" :key="index" :span="item.span || 12">
          <div class="view-item flex">
            ...
            <span v-else-if="item.field == 'abstract'" class="con flex-1">{{
              viewInfo['abstractCn'] || viewInfo['abstractEn'] || '-'
            }}</span>
      ...
      <ul>
        <li class="my-4 ml-2" v-for="(item, index) in list" :key="index"
          >[{{ index + 1 }}] {{ item }}</li
        >
      </ul> -->
    </div>
...
<script lang="ts">
  ...
  import { paperSchemas, reportSchemas } from './config/view';
  import {
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const viewInfo = ref();
      const docType = ref('');
      const schemasData: any = ref([]);
      const { createMessage } = useMessage();

      const getView = async () => {
        viewInfo.value = await domesGeneralInfoViewApi({ id: route.params.id });
        docType.value = viewInfo.value.docType;
        schemasData.value = docType.value == '期刊' ? paperSchemas : reportSchemas;
      };
      getView();
