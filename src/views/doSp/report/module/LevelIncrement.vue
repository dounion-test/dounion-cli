 ...
 * @LastEditTime: 2022-10-18 09:29:06
...
<script lang="ts">
  ...
  import { infoDocLevelIncrementApi } from '/@/api/doSp/report';
    ...
    setup() {
      ...
      const getInfoDocDocIncrement = async () => {
        ...
        list.value.forEach((item: any) => {
          item.children.forEach((it, index) => {
            it.firstLevel = item.firstLevel;
            it.isFirst = index ? false : true;
            it.count = item.children.length;
            tableDatas.push(it);
          });
        });
        tableData.value = tableDatas;
        await setTableData(tableDatas);
      };
      ...
      const filterData = () => {
        ...
        headerList.value.forEach((item: any) => {
          ...
          if (item.colKey == 'secondLevel') {
            dataObj.xData = data;
          }
          if (item.colKey != 'firstLevel' && item.colKey != 'secondLevel') {
