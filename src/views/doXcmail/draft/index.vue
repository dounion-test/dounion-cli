 ...
 * @LastEditTime: 2022-10-28 14:46:53
  ...
  <PageWrapper contentFullHeight contentBackground>
    <BasicTable @register="registerTable" class="!p-0" :searchInfo="searchParams">
      ...
      <template #topic="{ record }">
        <span
          :class="{ 'email-topic': hasPermission('XcmailDraftView') }"
...
<script lang="ts">
  ...
  import { columns, searchArr, getFormConfig } from '../config/draft';
  import { useMessage } from '/@/hooks/web/useMessage';
  import { useRouter } from 'vue-router';
  import { usePermission } from '/@/hooks/web/usePermission';
  export default defineComponent({
    name: 'XcmailDraftList',
    components: {
      Icon,
      BasicTable,
      TableAction,
    ...
    setup() {
      ...
      const searchParams = reactive({
        ...
        writeStartTime: undefined,
        writeEndTime: undefined,
      });
      const { hasPermission } = usePermission();
      const hasAuth = [hasPermission('XcmailDraftEdit'), hasPermission('XcmailDraftView')];
      /**
       * table 列表
       */
      const [registerTable, { reload }] = useTable({
        api: oamailDraftListApi,
        rowKey: 'id',
        columns,
        ...
        customRow: (record) => {
          return {
            onDblclick: () => {
              if (hasAuth[0]) {
                router.push({ name: 'XcmailDraftEdit', params: { id: record.id, type: 'edit' } });
              } else if (hasAuth[1]) {
                router.push({
                  name: 'XcmailDraftView',
                  params: { id: record.id, type: 'draft' },
                });
              } else {
      ...
      const handleView = (id) => {
        if (hasAuth[1]) {
          router.push({ name: 'XcmailDraftView', params: { id, type: 'draft' } });
        } else {
          return;
        }
      };
      ...
      const handleEdit = (item) => {
        router.push({ name: 'XcmailDraftEdit', params: { id: item.id, type: 'edit' } });
      };
      ...
      const handleDelete = async (record) => {
        await oamailDraftDeleteApi({ ids: record.id });
       ...
       * 彻底删除
       */
      const handleDeleteForever = async (record) => {
        await oamailDraftDeleteForeverApi({ ids: record.id });
        createMessage.success('操作成功');
        reload();
      };
        ...
        if (searchList.length > 0) {
          searchList.forEach((item) => {
            if (item.type == 'rangePicker') {
              searchParams.writeStartTime = item.value ? item.value[0] : undefined;
              searchParams.writeEndTime = item.value ? item.value[1] : undefined;
            } else {
